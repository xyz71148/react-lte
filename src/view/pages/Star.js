...
import {createInstance, fetchInstances} from "../../store/front/instanceActions";
...
class Star extends Component {
    ...
    onSelectCell({id, title, onAdd}) {
        if (['vip', "free", "order", "plan", "account"].includes(id) && !is_logged()) {
    ...
    render() {
        ...
        const size = "1.4em"
        const Icons = {
            "free": (<MdGamepad size={size}/>),
            "vip": (<MdSwapCalls size={size}/>),
